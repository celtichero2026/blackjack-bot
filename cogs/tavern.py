...
from config import TAVERN_CHANNEL_IDS, DAILY_REWARD
...
def is_tavern_channel(interaction):
    return interaction.channel_id in TAVERN_CHANNEL_IDS
...
def build_shot_bar_embed(user_id):
    ...
    embed.set_footer(text="Shots post publicly in The Tavern.")
    return embed
...
class Tavern(commands.Cog):
    ...
    async def tavern_event_loop(self):
        ...
        channels = [
            ...
            for channel_id in TAVERN_CHANNEL_IDS
        ]
        ...
        # One random Tavern receives the event each hour. This avoids doubling
        # rewards just because the bot is installed in multiple servers.
        ...
        embed.set_footer(text="Random Tavern Events roll once per hour with a 10% chance.")
